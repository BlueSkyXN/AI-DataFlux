...
class ModelConfig:
    ...
    def __init__(self, model_dict: dict[str, Any], channels: dict[str, Any]):
        ...
        self.safe_rps = float(model_dict.get("safe_rps", max(0.5, min(self.weight / 10, 10))))
        self.supports_json_schema = bool(model_dict.get("supports_json_schema", False))
        self.supports_advanced_params = bool(model_dict.get("supports_advanced_params", False))
        ...
        if self.channel_id not in channels:
            raise ValueError(f"模型 {self.id}: channel_id='{self.channel_id}' 在 channels 中不存在")
...
class ModelDispatcher:
    ...
    def select_model(
        ...
        # 获取可用模型及其权重
        available_models = [
            self.models[model_id] for model_id in available_ids
