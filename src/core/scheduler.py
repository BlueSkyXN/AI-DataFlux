...
class ShardedTaskManager:
    ...
    def calculate_optimal_shard_size(self, total_range: int) -> int:
        ...
        if self._process_info:
            try:
                ...
                memory_based_limit = int(
                    (available_mb * 0.3) / record_size_mb_estimate
                ) if record_size_mb_estimate > 0 else self.max_shard_size
            except psutil.Error as e:
        ...
        shard_size = max(
            self.min_shard_size, 
            min(calculated_size, self.max_shard_size)
        )
    ...
    def update_processing_metrics(
        ...
        if self.processing_metrics["avg_time_per_record"] == 0.0:
            ...
            self.processing_metrics["records_per_second"] = (
                1.0 / max(1e-9, self.processing_metrics["avg_time_per_record"])
            )
    ...
    def monitor_memory_usage(self) -> None:
        ...
        try:
            ...
            self.memory_tracker["peak_memory_usage"] = max(
                self.memory_tracker["peak_memory_usage"], 
                current_mem_mb
            )
    ...
    def finalize(self) -> None:
        ...
        for error_type, count in self.retried_tasks_count.items():
            ...
            logging.info(f"  - {error_type.value}: {count} 次重试 (最大重试次数: {max_retries})")
