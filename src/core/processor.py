...
class UniversalAIProcessor:
    ...
    def __init__(self, config_path: str):
        ...
        self.api_error_trigger_window = float(concurrency_cfg.get("api_error_trigger_window", 2.0))
    ...
    def cleanup_old_metadata(self, max_age_hours: int = 24) -> None:
        ...
        with self.metadata_lock:
            to_remove = [
                task_id for task_id, meta in self.task_metadata.items()
    ...
    def create_prompt(self, record_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            return self.prompt_template.replace("{record_json}", '{"error": "无法序列化数据"}')
    ...
    async def process_one_record_async(
        ...
        if not prompt:
            return {"_error": "prompt_generation_failed", "_error_type": ErrorType.SYSTEM}
    ...
    async def _process_loop(self, session: aiohttp.ClientSession) -> None:
        ...
        while True:
            ...
            for completed_task in done:
                try:
                    ...
                    if isinstance(result, dict):
                        ...
                        if error_type:
                            # 处理错误
                            api_error_in_batch = api_error_in_batch or (error_type == ErrorType.API)
            ...
            if api_error_in_batch:
                ...
                if current_time - self.last_api_pause_end_time > self.api_error_trigger_window:
                    logging.warning(f"检测到 API 错误，暂停 {self.api_pause_duration}s...")
